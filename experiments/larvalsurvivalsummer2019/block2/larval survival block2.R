setwd("/Users/michaeljardine/Desktop/DTP/Data and analysis/larvalsurvivalsummer2019/block2")
Lsurv <- read.csv("Larval survival block2 by date.csv")
str(Lsurv)
summary(Lsurv)
...
hist(Lsurv$females)
hist(Lsurv$Total)
# first two ok, total appears bunched to the right - try fixes
hist(Lsurv$Total)
hist(log(Lsurv$Total))
hist(sqrt(Lsurv$Total))
# is anything thats just worse
...
# while the total is bunched to right as before the two sexes look more normal by them selves
...
# mean propsurv = 
Lsurv$relsurv <- Lsurv$propsurv/0.5776
