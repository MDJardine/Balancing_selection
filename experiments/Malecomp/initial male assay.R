...
comp3 <-read.csv("block_3_ALT.csv")
...
## change Line to a factor
comp3$Line <- factor(comp$Line)
## change Round to a factor
comp3$Round <- factor(comp$Round)
...
## change pupa to a 1/0 score to see who the winner is
comp3$winner <- ifelse(comp3$Pupa %in% c("T"), 0, 1)
## winner: competitor = 0 (Tub), focal = 1 (Norm)
str(comp3)
summary(comp3)
...
## some simple plots
par(mfrow=c(2,2))
plot(winner ~ Allele, data=comp3)
plot(winner ~ Round, data=comp3)
plot(winner ~ Genotype, data=comp3)
plot(winner ~ Line, data=comp3)
## these don't seem greatly useful, probs they use the median and we only have 1/0 data - lsmeans plots may be better later

## some summary statistics for means and errors 
summarySE(comp3, measurevar=c("winner"), groupvars=c("Allele"))
summarySE(comp3, measurevar=c("winner"), groupvars=c("Line"))
summarySE(comp3, measurevar=c("winner"), groupvars=c("Genotype"))

## combine the line and the genotype to create 5B and 5D 
comp3$Linegenotype <- interaction(comp3$Line, comp3$Genotype)

summarySE(comp3, measurevar=c("winner"), groupvars=c("Linegenotype"))

## combine for genotype with introgressed part
comp3$Allelegenotype <- interaction(comp3$Allele, comp3$Genotype)

summarySE(comp3, measurevar=c("winner"), groupvars=c("Allelegenotype"))


### out 1st model will simply look at the i there are differences ebtween those flies with the male or female allele, taking into account the variation acros lines and day

GLMM3.1 <- glmer(winner ~ Allele + (1 | Round) + (1 | Line), data=comp3, family=binomial(link=logit))
...
summary(GLMM3.1)
anova(GLMM3.1)
wald.test(b=fixef(GLMM3.1), Sigma=vcov(GLMM3.1), Terms = 2, df=1)
...
GLMM3.2 <- glmer(winner ~ Allele*Genotype + (1 | Round) +(1 | Line), data=comp3, family=binomial(link=logit))
...
wald.test(b=fixef(GLMM3.2), Sigma=vcov(GLMM3.2), Terms = 2, df=1)
wald.test(b=fixef(GLMM3.2), Sigma=vcov(GLMM3.2), Terms = 3, df=1)
wald.test(b=fixef(GLMM3.2), Sigma=vcov(GLMM3.2), Terms = 4, df=1)
